...
where all the other values are $0$. The cycle index is
$$
...
We can further notice that Pólya's theorem essentially just applies Burnside's lemma for each individual weight.
...
The theorem can also be applied for various different counting problems, by changing the weights and subgroup of $S_n$.
